...
## Title
Clock Configuration, SysTick 10 kHz and Output Frequency Measurement

## Objective
To extend Lab 1 firmware with system clock setup logic, periodic interrupt generation, and measurable output signal toggling.

## Variant
Variant: **5**
Required frequency by task table: **80 MHz**.

## Task Requirements
1. Configure flash latency.
2. Configure PLL source and PLL factors.
3. Enable PLL and switch main clock source.
4. Start SysTick timer at 10 kHz.
5. Toggle GPIO pin inside SysTick handler.
6. Measure output signal frequency in simulation tools.

## Schematic / Simulation Setup
Runtime demonstration was assembled and executed in Wokwi.

- Scheme view:

![Lab 2 Wokwi scheme](Lab2_wokwi_scheme.png)

- Logic analyzer running view:

![Lab 2 logic analyzer](Lab2_wokwi_logic_analyzer.png)

- Frequency demonstration with active runtime state:

![Lab 2 frequency value](Lab2_wokwi_frequency_value.png)

## Source Code
Primary STM32 implementation file:
- `Lab2_main.c`

Wokwi runtime analogue sketch file:
- `Wokwi_Lab2_sketch.ino`

## Calculations (Variant 5)
Required target frequency from variant table:
- `f_SYSCLK = 80 MHz`

PLL setup used in STM32 code:
- `HSI = 16 MHz`
- `PLLM = 16`
- `PLLN = 320`
- `PLLP = 4`
...
Check:
1. `f_VCO_IN = 16 / 16 = 1 MHz`
2. `f_VCO_OUT = 1 * 320 = 320 MHz`
3. `f_SYSCLK = 320 / 4 = 80 MHz`

SysTick setup:
- required interrupt frequency: `10 kHz`
- reload value: `LOAD = 80,000,000 / 10,000 - 1 = 7999`

Output signal behavior:
- output pin toggles on each tick,
- expected square-wave fundamental frequency is approximately:
- `f_OUT ≈ 10 kHz / 2 = 5 kHz`

## Verification Notes
1. LED/button behavior from Lab 1 remains operational.
2. Output pin signal is generated continuously.
3. Logic analyzer confirms periodic toggling.
...
## Conclusion
The second laboratory task objectives were achieved: clock-configuration logic was implemented, 10 kHz periodic timing was established, and output frequency behavior was demonstrated and captured.
